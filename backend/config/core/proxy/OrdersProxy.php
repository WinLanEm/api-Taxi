...
class OrdersProxy
{
    ...
    {
        if(!isset($data['consumer_id']) || !isset($data['driver_id']) || !isset($data['trip_price']) || !isset($data['payment_method']) || !isset($data['payment']) || !isset($data['is_completed']) || !isset($data['waiting_price']) || !isset($data['source_street']) || !isset($data['source_house']) || !isset($data['final_street']) || !isset($data['final_house']) || !isset($data['city'])){
            http_response_code(400);
            $res = [
                'status' => false,
                'message' => 'Bad request',
        ...
        $this->order->setIsCompleted($is_completed);
    ...
    {
        if(!isset($data['id']) || !isset($data['consumer_id']) || !isset($data['driver_id']) || !isset($data['trip_price']) || !isset($data['payment_method']) || !isset($data['payment']) || !isset($data['is_completed']) || !isset($data['waiting_price']) || !isset($data['source_street']) || !isset($data['source_house']) || !isset($data['final_street']) || !isset($data['final_house']) || !isset($data['city'])){
            http_response_code(400);
            $res = [
                'status' => false,
                'message' => 'Bad request',
        ...
        $final_street_type = gettype($data['final_street']);
        $city_type = gettype($data['city']);
        if($id_type !== 'integer' || $consumer_id_type !== 'integer' || $driver_id_type !== 'integer' || $trip_price_type !== "integer" ||
            $payment_method_type !== 'string' || $payment_type !== 'boolean' || $is_completed_type !== 'boolean' ||
            $waiting_price_type !== 'integer' || $source_house_type !== 'integer' || $source_street_type !== 'string' ||
            $final_house_type !== 'integer' || $final_street_type !== 'string' || $city_type !== 'string'){
            http_response_code(400);
